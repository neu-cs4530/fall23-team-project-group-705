...
import Leaderboard, { PictionaryLeaderboard } from './Leaderboard';
...
describe('[T4] PictionaryLeaderboard', () => {
  // Spy on console.error and intercept react key warnings to fail test
  let consoleErrorSpy: jest.SpyInstance<void, [message?: any, ...optionalParms: any[]]>;
  beforeAll(() => {
    // Spy on console.error and intercept react key warnings to fail test
  ...
  function checkRow(row: HTMLElement, player: string, score?: number) {
    const columns = within(row).getAllByRole('gridcell');
    expect(columns).toHaveLength(2);
    expect(columns[0]).toHaveTextContent(player);
    if (score) expect(columns[1]).toHaveTextContent(score.toString());
  }
  beforeEach(() => {
    render(<PictionaryLeaderboard results={results} />);
  });
  it('should render the players in order of wins', () => {
    const rows = screen.getAllByRole('row');
    checkRow(rows[1], winner);
  ...
  it('should calculate the cumulative number of wins, losses, and ties for each player', () => {
    const rows = screen.getAllByRole('row');
    checkRow(rows[1], winner, 3);
    checkRow(rows[2], middle, 2);
    checkRow(rows[3], loser, 0);
  });
});
